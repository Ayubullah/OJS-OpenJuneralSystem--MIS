...
@section('page-description', 'Update journal information')
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-purple-50">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-xl font-bold text-gray-900">Edit Journal</h1>
                    <p class="text-sm text-gray-600">Update journal information</p>
                </div>
            </div>
        </div>

        <!-- Form -->
        <form action="{{ route('admin.journals.update', $journal) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="status" 
                    ...
                    <option value="active" {{ old('status', $journal->status) === 'active' ? 'selected' : '' }}>Active</option>
                    <option value="inactive" {{ old('status', $journal->status) === 'inactive' ? 'selected' : '' }}>Inactive</option>
                </select>
                @error('status')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
