...
@section('title', 'Create New Submission')
@section('page-title', 'Create Submission')
@section('page-description', 'Add a new submission to the system')
...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <!-- Form -->
        <form action="{{ route('admin.submissions.store') }}" method="POST" class="p-8 space-y-8">
            @csrf
            ...
            <div class="space-y-2">
                ...
                <textarea id="notes" 
                          name="notes" 
                          rows="4"
                          class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200 @error('notes') border-red-300 @enderror"
                          placeholder="Enter any additional notes about this submission">{{ old('notes') }}</textarea>
                @error('notes')
                <p class="text-sm text-red-600 flex items-center">
                    <i data-lucide="alert-circle" class="w-4 h-4 mr-1"></i>
                    {{ $message }}
