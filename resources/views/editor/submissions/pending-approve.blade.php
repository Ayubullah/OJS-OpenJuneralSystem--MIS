...
@section('title', __('Pending Approve'))
@section('page-title', __('Pending Approve'))
@section('page-description', __('Review files uploaded by authors for approval'))
...
<li class="flex items-center">
    <i data-lucide="chevron-right" class="w-4 h-4 text-gray-400 mx-2"></i>
    <span class="text-sm font-medium text-gray-500">{{ __('Pending Approve') }}</span>
</li>
@endsection
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
            <div class="flex items-center space-x-3">
                <h1 class="text-2xl font-bold text-gray-900">{{ __('Pending Approve') }}</h1>
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-purple-100 text-purple-800">
                    {{ __('Pending Approval Files') }}
                </span>
            </div>
            <p class="mt-1 text-sm text-gray-600">
                {{ __('Review files uploaded by authors for approval') }}
            </p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 bg-gradient-to-r from-purple-50 to-pink-50 border-b border-gray-200">
            <h2 class="text-lg font-bold text-gray-900">{{ __('Pending Approval Files') }} ({{ $submissions->total() }})</h2>
        </div>
                    ...
                    <div class="flex-1">
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500">Uploaded for approval</p>
                                    </div>
                                </div>
                        ...
                        <div class="ml-15 mt-4 flex items-center space-x-3">
                            <form action="{{ route('editor.submissions.approve-article', $submission) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" 
                                        onclick="return confirm('{{ __('Are you sure you want to approve this article? This will change the status to accepted.') }}')"
                            ...
                            <form action="{{ route('editor.submissions.reject-approval', $submission) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" 
                                        onclick="return confirm('{{ __('Are you sure you want to reject this approval request? The author will be notified.') }}')"
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-12 text-center">
        ...
        <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ __('No Pending Approval Files') }}</h3>
        <p class="text-sm text-gray-600">{{ __('There are no files waiting for approval at the moment.') }}</p>
    </div>
    @endif
</div>
