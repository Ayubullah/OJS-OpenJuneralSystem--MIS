...
@section('title', __('Approved Articles'))
@section('page-title', __('Approved Articles'))
@section('page-description', __('View all articles that have been approved through the approval workflow'))
...
<li class="flex items-center">
    <i data-lucide="chevron-right" class="w-4 h-4 text-gray-400 mx-2"></i>
    <span class="text-sm font-medium text-gray-500">{{ __('Approved') }}</span>
</li>
@endsection
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
            <div class="flex items-center space-x-3">
                <h1 class="text-2xl font-bold text-gray-900">{{ __('Approved Articles') }}</h1>
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">
                    {{ __('Approved Through Workflow') }}
                </span>
            </div>
            <p class="mt-1 text-sm text-gray-600">
                {{ __('View all articles that have been approved through the approval workflow') }}
            </p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 bg-gradient-to-r from-green-50 to-emerald-50 border-b border-gray-200">
            <h2 class="text-lg font-bold text-gray-900">{{ __('Approved Articles') }} ({{ $submissions->total() }})</h2>
        </div>
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex items-center space-x-4 mt-1 text-sm text-gray-600">
                                    ...
                                    <span class="flex items-center">
                                        ...
                                        {{ __('Approved on') }} {{ $submission->updated_at->format('M d, Y') }}
                                    </span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500">{{ __('Approved file') }}</p>
                                    </div>
                                </div>
                                <div class="flex items-center space-x-3">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-12 text-center">
        ...
        <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ __('No Approved Articles') }}</h3>
        <p class="text-sm text-gray-600">{{ __('There are no articles that have been approved through the approval workflow yet.') }}</p>
    </div>
    @endif
</div>
