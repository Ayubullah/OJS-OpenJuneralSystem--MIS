...
return new class extends Migration
{
    ...
    {
        // Add 'plagiarism' status to submissions table enum
    ...
    {
        // Remove 'plagiarism' status from submissions table enum
        DB::statement("ALTER TABLE submissions MODIFY COLUMN status ENUM('submitted', 'under_review', 'revision_required', 'disc_review', 'pending_verify', 'verified', 'accepted', 'published', 'rejected') DEFAULT 'submitted'");
        
        // Also remove from articles table if it exists
        if (Schema::hasTable('articles')) {
            DB::statement("ALTER TABLE articles MODIFY COLUMN status ENUM('submitted', 'under_review', 'revision_required', 'disc_review', 'pending_verify', 'verified', 'accepted', 'published', 'rejected') DEFAULT 'submitted'");
        }
    }
};
